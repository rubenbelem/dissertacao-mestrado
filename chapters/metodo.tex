...
Essa restrição faz com que a busca binária passe a ser realizada em somente $15\% \sim 20\%$ do total de nós ativos de $\Psi_{p[1..\lambda]}$, em contraste aos $80\% \sim 90\%$ do método IP2LB. O efeito disso é que o tempo de processamento de consultas do IP2LRB aproxima-se do tempo do método IP2L, porém trazendo mais resultados que antes estavam faltando com o IP2LB. No entanto, mesmo com essa restrição o IP2LRB não consegue ter acurácia absoluta, pois ainda não traz $100\%$ dos resultados que deveria. A seção~\ref{sec:results} contém mais detalhes sobre a diferença de acurácia entre os métodos.
